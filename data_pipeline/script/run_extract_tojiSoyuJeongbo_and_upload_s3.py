...
EXTRACT_SCRIPT = PROJECT_ROOT / "data_pipeline" / "extract" / "extract_tojiSoyuJeongbo.py"
RUN_RANGE_PATTERN = re.compile(r"(\d{4}-\d{2}-\d{2})_to_(\d{4}-\d{2}-\d{2})$")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run extract_tojiSoyuJeongbo.py and upload written output directories to S3 "
...
def parse_run_range(name: str) -> Optional[tuple[date, date]]:
    m = RUN_RANGE_PATTERN.fullmatch(name)
    if not m:
        return None
...
def find_latest_run_range_name(*roots: Path) -> Optional[str]:
    ...
    for root in roots:
        ...
        for d in root.iterdir():
            ...
            if d.name in seen:
                continue
            parsed = parse_run_range(d.name)
            if parsed is None:
                continue
            start, end = parsed
            candidates.append((end, start, d.name))
            seen.add(d.name)
...
def resolve_output_dirs() -> list[Path]:
    base = PROJECT_ROOT / "data" / "tojiSoyuJeongbo"
    zip_root = base / "_work" / "per_row_zips"
    unzip_root = base / "_work" / "unzipped"

    dirs: list[Path] = []

    latest_run_name = find_latest_run_range_name(zip_root, unzip_root)
    if latest_run_name is not None:
        dirs.append(zip_root / latest_run_name)
        dirs.append(unzip_root / latest_run_name)
...
def main() -> None:
    ...
    print("[INFO] extract_tojiSoyuJeongbo.py finished successfully")
