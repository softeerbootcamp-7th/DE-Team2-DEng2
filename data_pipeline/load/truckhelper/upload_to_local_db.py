...
from api.models.truckhelper import TruckhelperParkingArea
...
TRUCKHELPER_CONFIG = TableConfig(
    ...
    column_aliases={},
    float_columns=frozenset({"lat", "lon"}),
    auto_columns=frozenset(),
    snapshot_keys=(),  # PK/year/month 없음 → 전체 테이블 교체
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="트럭헬퍼 차고지 Parquet → 로컬 Docker PostgreSQL 적재",
    )
...
def main() -> None:
    ...
    # 3) DB 연결 및 적재 (snapshot 빈 dict → 전체 테이블 교체)
